...
<script type="text/javascript">
	...
	*/
	function validateForm(form) {
			//유효성검사 결과 testForm은 submit
			//체크 로직을 모두 통과했다면 서버에 요청, 체크로직을 통과하지 못했다면 메세지 처리
			if(form.kor.value =='') {
				alert('국어점수를 입력해주세요');
				form.kor.focus();
				return false;
...
<form name="testForm">
	...
	<!-- 폼에 버튼이 하나만 있다면 이 버튼은 서브밋 버튼이 됩니다.
		서브밋 버튼은 폼을 전송하는 기본이벤트가 정의되어 있습니다.
		
		유효성검사(validation체크)를 진행하기 위해서는
		기본 이벤트를 제거하는 작업을 해야함 
	-->
	<button id="submitBtn">학점구하기</button>
</form>
<!-- 동일한 페이지가 다시 요청되어 form 태그 안에 요소를 출력 -->
<!-- 국어, 여어, 수학 점수가 모두 입력 되었을때만 출력함. empty -->
