...
<head>
	...
	<script>
		window.onload = function() {
			...
			/*변수 선언
			var - 전역변수,변수의 중복선언이 가능
			let - 지역변수
			const - 지역 상수
			*/
			let regBtn = document.querySelector("#regBtn");
			//익명의 함수를 작성할수도 있고 이미 작성된 함수의 이름을 넘겨줄수도 있다.
			//함수의 이름을 넣어줄땡는 함수 이름 뒤에 ()를 붙이면 안됨.
			//함수이름() : 함수를 실행하라는 의미
			...
			function validationCheck() {
				...
				if(email.value.length > 100) {
					alert("이메일의 길이는 100자를 초과할수 없습니다.");
					email.focus();
					return false; 
				}
				//정규식 패턴 문자열을 생성
				...
				if(pw.value.length > 10) {
					alert("비밀번호는 10자리를 초과할수 없습니다.");
					pw.focus();
					return false; 
				}
				if(pw.value != pwCheck.value) {
					alert("비밀번호가 일치하지 않습니다. 비밀번호를 인해주세요");
					pw.focus();
					return false; 
				}
				//등록 요청
                        ...
                        <div class="p-5">
                            ...
                            <form name = "regForm" class="user">
                                ...
                                <div class="form-group">
                                    ...
                                    id="email" name = "email" placeholder="email 입력">
                                </div>
